...
if util.download_to_file(csv_url, in_file):
    ...
    cur.execute('COMMIT;')
    if rows_added == 1:
        print("Added 1 new entry to gcmd_data_format.")
    else:
        print("Added %d new entries to gcmd_data_format." % (rows_added,))
else:
    print("Error downloading file from \"%s\" to \"%s\"" % (csv_url, in_file))
